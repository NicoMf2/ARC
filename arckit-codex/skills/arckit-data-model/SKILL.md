...
1. **Read existing artifacts from the project context:**
   ...
   **MANDATORY** (warn if missing):
   - **REQ** (Requirements)
     ...
     - If missing: STOP and warn user to run `/prompts:arckit.requirements` first — data model MUST be based on DR-xxx requirements
...
4. **Read the template** (with user override support):
   ...
   > **Tip**: Users can customize templates with `/prompts:arckit.customize data-model`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.data-model` command |
```

### Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/prompts:arckit.data-model` command
**Generated on**: {DATE}
...
- Suggested next steps (e.g., "Review data model with data protection officer before proceeding to HLD" or "Run `/prompts:arckit.hld-review` to validate database technology choices")

## Example Usage

User: `/prompts:arckit.data-model Create data model for payment gateway project`
...
- **Output**: Feeds into `/prompts:arckit.hld-review` (validates database technology choices)
- **Output**: Feeds into `/prompts:arckit.dld-review` (validates schema design, indexes, query patterns)
- **Output**: Feeds into `/prompts:arckit.sow` (RFP includes data migration, data governance requirements)
- **Output**: Supports `/prompts:arckit.traceability` (DR-xxx → Entity → Attribute → HLD Component)
...
- Run `/prompts:arckit.research` to research database technologies
- Run `/prompts:arckit.hld-review` after HLD is created
```
...
- `/prompts:arckit.hld-review` -- Validate database technology choices
- `/prompts:arckit.dld-review` -- Validate schema design and query patterns
- `/prompts:arckit.sow` -- Include data migration and governance in RFP
- `/prompts:arckit.traceability` -- Map DR-xxx to entities and attributes
