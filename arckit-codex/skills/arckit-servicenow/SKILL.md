...
# /prompts:arckit.servicenow - ServiceNow Service Design Command
...
Use `/prompts:arckit.servicenow` after completing:

1. Requirements (`/prompts:arckit.requirements`)
2. Architecture diagrams (`/prompts:arckit.diagram`) - especially C4 diagrams
3. High-Level Design (HLD) or Detailed Design (DLD) - if available
4. Technology Code of Practice assessment (`/prompts:arckit.tcop`) - for UK Gov projects
...
- **DIAG** (Architecture Diagrams) in `projects/{project-name}/diagrams/`
  ...
  - If missing: warn user to run `/prompts:arckit.diagram` first
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize servicenow`
...
```text
/prompts:arckit.servicenow Generate ServiceNow design for the DWP Benefits Eligibility Chatbot - this is a Tier 1 critical service requiring 24/7 support
```
...
```text
/prompts:arckit.servicenow Create ServiceNow design for the payment processing service
```
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.servicenow` command |
```

### Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/prompts:arckit.servicenow` command
**Generated on**: {DATE}
...
"⚠️ Cannot find requirements document (ARC-*-REQ-*.md). Please run `/prompts:arckit.requirements` first. ServiceNow design requires NFRs for SLA definitions."

### If No Architecture Diagrams Found

"⚠️ Cannot find architecture diagrams. Please run `/prompts:arckit.diagram context` and `/prompts:arckit.diagram container` first. ServiceNow design requires architecture diagrams for CMDB structure."
