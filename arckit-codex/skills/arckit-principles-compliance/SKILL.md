...
> **Tip**: Users can customize templates with `/prompts:arckit.customize principles-compliance`
...
if [ ! -f projects/000-global/ARC-000-PRIN-*.md ]; then
    ERROR "Architecture principles not found. Run /prompts:arckit.principles first."
fi
```
...
**Generated by**: ArcKit `/prompts:arckit.principles-compliance` command
...
After generating the assessment document, **suggest follow-up commands**:
   ...
   ```text
   📋 **Related Commands**:
   - /prompts:arckit.analyze - Run comprehensive gap analysis
   - /prompts:arckit.hld-review - Review vendor HLD against principles
   - /prompts:arckit.dld-review - Review vendor DLD against principles
   - /prompts:arckit.service-assessment - GDS Service Standard assessment (UK Gov)
   ```
