...
- **REQ** (Requirements) — primary source
  ...
  - If missing: warn user to run `/prompts:arckit.requirements` first — backlog is derived from requirements
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.backlog` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/prompts:arckit.backlog` command
...
📚 Integration:
  ...
  - Link to /prompts:arckit.traceability for requirements tracking
```
...
Cannot generate backlog without requirements. Please run:
  /prompts:arckit.requirements

Then re-run /prompts:arckit.backlog
```
...
For better component mapping, run:
  /prompts:arckit.hld or /prompts:arckit.diagram
...
```bash
/prompts:arckit.backlog VELOCITY=25 SPRINTS=12
```
...
```bash
/prompts:arckit.backlog FORMAT=all
```
...
```bash
/prompts:arckit.backlog PRIORITY=risk
```
...
- `/prompts:arckit.requirements` → All stories
- `/prompts:arckit.hld` → Component mapping
- `/prompts:arckit.stakeholders` → User personas
- `/prompts:arckit.risk-register` → Risk priorities
- `/prompts:arckit.threat-model` → Security stories
- `/prompts:arckit.business-case` → Value priorities
- `/prompts:arckit.principles` → Definition of Done

### Outputs To

- `/prompts:arckit.traceability` → Requirements → Stories → Sprints
- `/prompts:arckit.test-strategy` → Test cases from acceptance criteria
- `/prompts:arckit.analyze` → Backlog completeness check
...
- `/prompts:arckit.trello` -- Export backlog to Trello board
- `/prompts:arckit.traceability` -- Map user stories back to requirements
