...
- `ARC-*-REQ-*.md` in `projects/{project}/` — Requirements specification
  ...
  - If missing: STOP and report that `/prompts:arckit.requirements` must be run first
...
```text
**Generated by**: ArcKit `/prompts:arckit.datascout` agent
**Generated on**: {DATE}
...
- Next steps (run `/prompts:arckit.data-model`, `/prompts:arckit.adr`, `/prompts:arckit.dpia`)
...
- **No requirements found**: Stop immediately, tell user to run `/prompts:arckit.requirements`
...
- `/prompts:arckit.data-model` -- Add discovered sources to data model
- `/prompts:arckit.research` -- Research data source pricing and vendors
- `/prompts:arckit.adr` -- Record data source selection decisions
- `/prompts:arckit.dpia` -- Assess third-party data sources with personal data
- `/prompts:arckit.diagram` -- Create data flow diagrams
- `/prompts:arckit.traceability` -- Map DR-xxx requirements to discovered sources
