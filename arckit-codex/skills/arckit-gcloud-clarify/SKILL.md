...
After using `/prompts:arckit.gcloud-search` to find G-Cloud services, you have a shortlist but face challenges:
...
- **REQ** (Requirements) — Extract: All MUST requirements (BR-xxx, FR-xxx, NFR-xxx, INT-xxx, DR-xxx), SHOULD requirements, compliance (NFR-C-xxx), integration (INT-xxx), performance (NFR-P-xxx), security (NFR-S-xxx)
  - If missing: ERROR "Run `/prompts:arckit.requirements` first — need source requirements"
- **GCLD** (G-Cloud Search, in procurement/) — Extract: Shortlisted services (top 3-5), service names, supplier names, service links, key features, Must-Have Match scores, Desirable Features scores, compliance mentions, pricing
  - If missing: ERROR "Run `/prompts:arckit.gcloud-search` first — need service search results"
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.gcloud-clarify` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/prompts:arckit.gcloud-clarify` command
...
7. ✅ **Final evaluation**:
   - [ ] Use `/prompts:arckit.evaluate` to score suppliers
...
**Parallel Activity**: While waiting for responses, prepare evaluation criteria with `/prompts:arckit.evaluate`.
...
**Generated**: [DATE]
**Tool**: /prompts:arckit.gcloud-clarify
**Next Command**: `/prompts:arckit.evaluate` (after supplier responses received)
```
...
5. Use /prompts:arckit.evaluate after receiving responses to score and compare
...
- **No ARC-*-REQ-*.md**: ERROR "Requirements not found - run /prompts:arckit.requirements first"
- **No gcloud-ARC-*-REQ-*.md**: ERROR "G-Cloud search results not found - run /prompts:arckit.gcloud-search first"
- **No services shortlisted**: ERROR "No services to clarify - gcloud-search found no results"
- **All MUST requirements confirmed**: INFO "All MUST requirements confirmed with evidence - minimal clarification needed. Proceed to /prompts:arckit.evaluate"

## Integration with Workflow

**Complete G-Cloud Procurement Workflow**:

1. `/prompts:arckit.requirements` → Define service needs
2. `/prompts:arckit.gcloud-search` → Find services on Digital Marketplace
3. **`/prompts:arckit.gcloud-clarify`** → Identify gaps, generate questions
4. *Supplier engagement* → Send questions, receive responses
5. `/prompts:arckit.evaluate` → Score suppliers based on responses
