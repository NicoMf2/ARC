...
- **PRIN** (Architecture Principles, in 000-global) — Extract: Strategic principles, technology standards, compliance requirements, cloud-first policy
  - If missing: warn user to run `/prompts:arckit.principles` first
- **REQ** (Requirements) — Extract: Business requirements, functional requirements, non-functional requirements. Identify: User needs, capabilities, technical components
  - If missing: warn user to run `/prompts:arckit.requirements` first
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize wardley`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.wardley` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/prompts:arckit.wardley` command
...
"I recommend running `/prompts:arckit.requirements` first to establish requirements before creating a Wardley Map."

# If no architecture principles exist
"I recommend running `/prompts:arckit.principles` first to establish architecture principles."
```
...
"Based on your Wardley Map, I recommend running `/prompts:arckit.sow` to create an RFP for vendor procurement."

# If map shows build vs buy decisions needed
"Your map identifies several build vs buy decisions. Consider running `/prompts:arckit.evaluate` to compare vendor options."

# If map shows UK Government project
"As a UK Government project, I recommend running `/prompts:arckit.tcop` to assess Technology Code of Practice compliance."

# If map shows AI components with HIGH-RISK
"Your map includes HIGH-RISK AI components. I recommend running `/prompts:arckit.ai-playbook` and `/prompts:arckit.atrs`."
```
...
```bash
"/prompts:arckit.hld-review Review HLD against Wardley Map strategic positioning"
```
...
The `/prompts:arckit.analyze` command should include Wardley Map validation:
...
1. Run `/prompts:arckit.sow` to create RFP for G-Cloud procurement (AWS, GPT-4, Auth0)
2. Run `/prompts:arckit.ai-playbook` to complete AI Playbook assessment
3. Run `/prompts:arckit.atrs` to generate ATRS record
4. Run `/prompts:arckit.tcop` to validate TCoP compliance (Cloud First, Open Standards, Reuse)
...
- /prompts:arckit.sow - Generate RFP for vendor procurement
- /prompts:arckit.tcop - Assess Technology Code of Practice compliance
- /prompts:arckit.ai-playbook - Assess AI Playbook compliance (if AI components)
```
...
- `/prompts:arckit.roadmap` -- Create strategic roadmap from evolution analysis
- `/prompts:arckit.strategy` -- Synthesise Wardley insights into architecture strategy
- `/prompts:arckit.research` -- Research vendors for Custom-Built components *(when Custom-Built components identified that need market research)*
