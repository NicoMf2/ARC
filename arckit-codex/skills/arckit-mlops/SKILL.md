...
# /prompts:arckit.mlops - MLOps Strategy Command
...
Use `/prompts:arckit.mlops` when your project includes:
...
1. Requirements (`/prompts:arckit.requirements`) - to understand ML use cases
2. Data model (`/prompts:arckit.data-model`) - to understand training data
3. AI Playbook assessment (`/prompts:arckit.ai-playbook`) - for governance context (UK Gov)
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize mlops`
...
"⚠️ Data model document (ARC-*-DATA-*.md) not found. Training data understanding is important for MLOps. Consider running `/prompts:arckit.data-model` first."
...
```markdown
---
**Generated by**: ArcKit `/prompts:arckit.mlops` command
**Generated on**: [DATE]
