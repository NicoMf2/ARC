...
# /prompts:arckit.devops - DevOps Strategy Command
...
Use `/prompts:arckit.devops` after completing:

1. Requirements (`/prompts:arckit.requirements`) - for deployment and performance needs
2. Architecture diagrams (`/prompts:arckit.diagram`) - for deployment topology
3. Research (`/prompts:arckit.research`) - for technology stack decisions
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize devops`
...
"⚠️ Cannot find requirements document (ARC-*-REQ-*.md). Please run `/prompts:arckit.requirements` first. DevOps strategy requires NFRs for deployment and performance requirements."

### If No Architecture Principles

"⚠️ Architecture principles not found. Using cloud-agnostic defaults. Consider running `/prompts:arckit.principles` to establish technology standards."
...
```markdown
---
**Generated by**: ArcKit `/prompts:arckit.devops` command
**Generated on**: [DATE]
