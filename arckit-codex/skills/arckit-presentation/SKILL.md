...
Run more /arckit commands first, then re-run /prompts:arckit.presentation.
```
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize presentation`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.presentation` command | [PENDING] | [PENDING] |
```

#### Step 7e: Populate Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/prompts:arckit.presentation` command
...
User: `/prompts:arckit.presentation Generate executive presentation for NHS appointment booking`
...
- If no artifacts exist, recommend running `/prompts:arckit.plan` or `/prompts:arckit.requirements` first
...
- Run `/prompts:arckit.story` for a full narrative companion document
```
