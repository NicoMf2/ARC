...
1. **Architecture Principles** (REQUIRED):
   ...
   - If it does NOT exist:
     ...
     Please run: /prompts:arckit.principles Create enterprise architecture principles
...
2. **Data Model** (HIGHLY RECOMMENDED):
   ...
   - If it does NOT exist:
     ...
     Consider running: /prompts:arckit.data-model Create data model for [project name]
   ...
   - If user says "no", stop here and tell them to run `/prompts:arckit.data-model` first
...
3. **Stakeholder Analysis** (RECOMMENDED):
   ...
   - If it does NOT exist:
     ...
     Consider running: /prompts:arckit.stakeholders Analyze stakeholders for [project name]
...
- /prompts:arckit.stakeholders - Identify domain owners and consumers
- /prompts:arckit.data-model - Define entities that become data products
- /prompts:arckit.requirements - Capture DR-xxx data requirements
```
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize data-mesh-contract`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.data-mesh-contract` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/prompts:arckit.data-mesh-contract` command
...
- `/prompts:arckit.traceability` - Link this contract to requirements and consumers
- `/prompts:arckit.analyze` - Score contract completeness and governance quality
- `/prompts:arckit.dpia` - Generate Data Protection Impact Assessment (if PII present)
...
```text
💡 Tip: Run /prompts:arckit.requirements to capture DR-xxx data requirements.
...
```text
💡 Tip: Run /prompts:arckit.stakeholders to identify domain owners and consumers.
...
Consider running: /prompts:arckit.dpia Generate DPIA for {project_name}
```
...
Consider running:
   ...
   - /prompts:arckit.service-assessment - GDS Service Standard (if digital service)
```
...
```text
User: /prompts:arckit.data-mesh-contract Create contract for customer payments
Assistant:
...
```text
User: /prompts:arckit.data-mesh-contract seller-analytics contract
Assistant:
  - Checks prerequisites ✅
  - Warns: No data model found
  - User confirms to proceed
  - Generates contract with generic schema placeholders
  - Recommends running /prompts:arckit.data-model first
```

**Example 3: Contract with full context**

```text
User: /prompts:arckit.data-mesh-contract fraud-detection-features
Assistant:
  - Checks prerequisites ✅
...
- Stop and tell user to run `/prompts:arckit.principles` first
