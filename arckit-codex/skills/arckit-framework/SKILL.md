...
**Generated by**: ArcKit `/prompts:arckit.framework` agent
**Generated on**: {DATE}
...
Include the generation metadata footer (same format as FWRK overview but referencing `/prompts:arckit.framework` agent).
...
- Next steps (suggest `/prompts:arckit.pages` to publish, or additional commands to fill gaps in coverage)
...
- **No requirements found**: Note this gap prominently in the Executive Summary. Suggest running `/prompts:arckit.requirements`
- **No principles found**: Note this gap. Suggest running `/prompts:arckit.principles`
...
- `/prompts:arckit.glossary` -- Generate glossary of framework terminology
- `/prompts:arckit.maturity-model` -- Create maturity model for framework adoption
