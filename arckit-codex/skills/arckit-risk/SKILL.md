...
- **After**: `/prompts:arckit.stakeholders` (MANDATORY - every risk needs an owner)
- **Before**: `/prompts:arckit.sobc` (SOBC Management Case Part E uses risk register)
...
1. **Read existing artifacts** from the project context:
   ...
   **MANDATORY** (warn if missing):
   - **STKE** (Stakeholder Analysis) — Extract: risk owners from RACI matrix, affected stakeholders, conflict analysis (conflicts ARE risks), stakeholder drivers (drivers under threat = strategic risks)
     - If missing: STOP and warn user to run `/prompts:arckit.stakeholders` first — every risk MUST have an owner
...
7. **For EACH risk identified, create comprehensive risk profile**:
   ...
   **Read the template** (with user override support):
   ...
   > **Tip**: Users can customize templates with `/prompts:arckit.customize risk-register`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.risk` command |
```

### Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/prompts:arckit.risk` command
**Generated on**: {DATE}
...
- Tell user: "Risk register requires stakeholder analysis to identify risk owners and affected parties. Please run `/prompts:arckit.stakeholders` first."
...
- `/prompts:arckit.sobc` -- Feed risk register into SOBC Management Case
- `/prompts:arckit.requirements` -- Create risk-driven requirements
- `/prompts:arckit.secure` -- Validate security controls against risks
