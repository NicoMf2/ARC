...
Reads the JSON backlog produced by `/prompts:arckit.backlog FORMAT=json` and pushes it to Trello:
...
- This is produced by `/prompts:arckit.backlog FORMAT=json`
...
Please generate one first:
  /prompts:arckit.backlog FORMAT=json

Then re-run /prompts:arckit.trello
```
...
- `/prompts:arckit.backlog FORMAT=json` - Backlog JSON file (MANDATORY)
