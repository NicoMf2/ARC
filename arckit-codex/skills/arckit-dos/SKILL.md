...
> **Tip**: Users can customize templates with `/prompts:arckit.customize dos`
...
- **PRIN** (Architecture Principles, in 000-global) — Extract: technology standards, governance constraints for vendor proposals
  - If missing: ERROR — run `/prompts:arckit.principles` first to define governance standards
- **REQ** (Requirements) — Extract: BR/FR/NFR/INT/DR IDs, priorities, acceptance criteria — source of truth for DOS
  - If missing: ERROR — run `/prompts:arckit.requirements` first to define project needs

**RECOMMENDED** (read if available, note if missing):

- **STKE** (Stakeholder Analysis) — Extract: user personas, business drivers, evaluation priorities
  - If missing: WARN — consider running `/prompts:arckit.stakeholders` to understand stakeholder priorities
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.dos` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/prompts:arckit.dos` command
...
Reference: Run `/prompts:arckit.hld-review` and `/prompts:arckit.dld-review` for formal review processes
...
Reference: `/prompts:arckit.traceability` for traceability matrix generation and validation
...
Reference: Generate with `/prompts:arckit.diagram`, `/prompts:arckit.data-model`
...
2. **Prepare Review Frameworks**:
   - Run `/prompts:arckit.hld-review` to set up HLD review process
   - Run `/prompts:arckit.dld-review` to set up DLD review process
...
4. **Traceability Setup**:
   - Run `/prompts:arckit.traceability` to establish tracking framework
...
- **`/prompts:arckit.evaluate`**: Create vendor evaluation framework and scoring
- **`/prompts:arckit.hld-review`**: High-Level Design review process for vendor deliverables
- **`/prompts:arckit.dld-review`**: Detailed Design review process for vendor deliverables
- **`/prompts:arckit.traceability`**: Requirements traceability matrix validation
...
- /prompts:arckit.evaluate - Create vendor evaluation framework after receiving proposals
- /prompts:arckit.hld-review - Set up HLD review process for vendor deliverables
- /prompts:arckit.dld-review - Set up DLD review process for vendor deliverables
- /prompts:arckit.traceability - Validate requirements traceability with vendor
...
- **No principles**: ERROR "Run /prompts:arckit.principles first - governance standards required"
- **No requirements**: ERROR "Run /prompts:arckit.requirements first - nothing to procure"
- **No project**: Suggest the user run `/prompts:arckit.init` or provide a project name to create one
- **Wrong framework**: If user mentions G-Cloud or cloud services, suggest `/prompts:arckit.gcloud-search` instead
