...
# /prompts:arckit.operationalize - Operational Readiness Command
...
Use `/prompts:arckit.operationalize` after completing:

1. Requirements (`/prompts:arckit.requirements`) - for SLA targets
2. Architecture diagrams (`/prompts:arckit.diagram`) - for component inventory
3. HLD/DLD review (`/prompts:arckit.hld-review` or `/prompts:arckit.dld-review`) - for technical details
4. Data model (`/prompts:arckit.data-model`) - for data dependencies

Run this command **before go-live** to ensure operational readiness. This is complementary to `/prompts:arckit.servicenow` (which focuses on ITSM tooling) - this command focuses on the operational practices and documentation.
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize operationalize`
...
"⚠️ Cannot find requirements document (ARC-*-REQ-*.md). Please run `/prompts:arckit.requirements` first. Operational readiness requires NFRs for SLO definitions."

### If No Architecture Diagrams

"⚠️ Cannot find architecture diagrams. Runbooks require component inventory. Please run `/prompts:arckit.diagram container` first."
...
```markdown
---
**Generated by**: ArcKit `/prompts:arckit.operationalize` command
**Generated on**: [DATE]
