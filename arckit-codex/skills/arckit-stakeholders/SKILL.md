...
2. **Read the template** (with user override support):
   ...
   > **Tip**: Users can customize templates with `/prompts:arckit.customize stakeholder-drivers`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.stakeholders` command |
```

### Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/prompts:arckit.stakeholders` command
**Generated on**: {DATE}
...
8. **Summarize what you created**:
   ...
   - Suggested next steps: "Now run `/prompts:arckit.requirements` to create requirements that align with and address these stakeholder goals and drivers"
...
```text
/prompts:arckit.stakeholders Analyze stakeholders for a cloud migration project where the CFO wants cost savings, the CTO wants innovation, Operations is worried about downtime, and Security needs enhanced controls
```
...
```text
/prompts:arckit.stakeholders Analyze stakeholders for a DWP benefits chatbot where the Minister wants quick delivery, Civil Service wants due diligence, Citizens need accuracy, and ICO requires data protection
```
...
- `/prompts:arckit.requirements` -- Create requirements aligned to stakeholder goals
- `/prompts:arckit.risk` -- Create risk register with stakeholder risk owners
- `/prompts:arckit.sobc` -- Build business case from stakeholder drivers
