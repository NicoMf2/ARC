...
- **PRIN** (Architecture Principles, in `projects/000-global/`)
  ...
  - If missing: STOP and ask user to run `/prompts:arckit.principles` first. The roadmap must align to approved principles.
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize roadmap`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.roadmap` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/prompts:arckit.roadmap` command
...
4. Create detailed project plan for Phase 1: `/prompts:arckit.plan`
5. Prepare business case (SOBC): `/prompts:arckit.sobc`
6. Generate backlog from roadmap: `/prompts:arckit.backlog`
...
7. **Integration**: Roadmap feeds into:
   - `/prompts:arckit.plan` - Detailed plans for each phase
   - `/prompts:arckit.sobc` - Business case aligned to roadmap investment
   - `/prompts:arckit.backlog` - User stories prioritized by roadmap timeline
   - `/prompts:arckit.traceability` - Full traceability matrix
...
- `/prompts:arckit.backlog` -- Generate product backlog from roadmap
- `/prompts:arckit.plan` -- Create detailed project plan for Phase 1
