...
> **Tip**: Users can customize templates with `/prompts:arckit.customize diagram`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.diagram` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/prompts:arckit.diagram` command
...
"I recommend running `/prompts:arckit.requirements` first to establish requirements before creating diagrams."

# If no HLD exists
"For a container diagram, I recommend having an HLD first. Run `/prompts:arckit.hld-review` or create HLD documentation."

# If no Wardley Map exists
"For strategic context (build vs buy), consider running `/prompts:arckit.wardley` first."
```
...
- Run `/prompts:arckit.hld-review` to create technical architecture
- Run `/prompts:arckit.diagram container` to show technical containers"

# After Container diagram
"Your container diagram is ready. Next steps:
- Run `/prompts:arckit.dld-review` for detailed component design
- Run `/prompts:arckit.diagram component` to show internal structure
- Run `/prompts:arckit.diagram deployment` to show infrastructure"

# After Deployment diagram
"Your deployment diagram is ready. Consider:
- Running `/prompts:arckit.tcop` to validate Cloud First compliance
...
```bash
"/prompts:arckit.hld-review Review HLD with container diagram validation"
```
...
The `/prompts:arckit.traceability` command should include diagram references:
...
/prompts:arckit.requirements Build a payment gateway...

# 2. Create context diagram (shows system boundary)
/prompts:arckit.diagram context Generate context diagram for payment gateway

# 3. Create Wardley Map (strategic positioning)
/prompts:arckit.wardley Create Wardley Map showing build vs buy

# 4. Create HLD
/prompts:arckit.hld-review Create high-level design

# 5. Create container diagram (technical architecture)
/prompts:arckit.diagram container Generate container diagram showing technical architecture
```
...
/prompts:arckit.diagram container Generate container diagram from Acme Corp HLD

# 3. Review HLD with diagram validation
/prompts:arckit.hld-review Review Acme Corp HLD against container diagram
```
...
/prompts:arckit.diagram component Generate component diagram for Payment API container

# 2. Create sequence diagram for key flows
/prompts:arckit.diagram sequence Generate sequence diagram for payment processing flow

# 3. Review DLD
/prompts:arckit.dld-review Review detailed design with component and sequence diagrams
```
...
/prompts:arckit.diagram deployment Generate AWS deployment diagram showing Cloud First compliance

# 2. Create data flow diagram
/prompts:arckit.diagram dataflow Generate data flow diagram showing UK GDPR PII handling

# 3. Assess TCoP compliance
/prompts:arckit.tcop Assess TCoP compliance with deployment and data flow diagrams
```
...
- /prompts:arckit.hld-review - Review HLD against this diagram
- /prompts:arckit.traceability - Validate requirements coverage
- /prompts:arckit.analyze - Comprehensive governance quality check
```
