...
- **PRIN** (Architecture Principles, in 000-global) — Extract: Guiding principles, decision framework, technology standards
  - If missing: STOP and ask user to run `/prompts:arckit.principles` first. Strategy must be grounded in principles.
- **STKE** (Stakeholder Analysis) — Extract: Stakeholder drivers, goals, measurable outcomes, conflicts, engagement strategies
  - If missing: STOP and ask user to run `/prompts:arckit.stakeholders` first. Strategy must address stakeholder drivers.
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize strategy`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/prompts:arckit.strategy` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/prompts:arckit.strategy` command
...
4. Create detailed requirements: `/prompts:arckit.requirements`
5. Expand roadmap: `/prompts:arckit.roadmap`
6. Begin detailed design: `/prompts:arckit.diagram`
...
8. **Integration with Other Commands**:
   - Strategy feeds into: `/prompts:arckit.requirements` (detailed requirements), `/prompts:arckit.roadmap` (expanded timeline), `/prompts:arckit.plan` (project delivery)
   - Strategy is informed by: `/prompts:arckit.principles`, `/prompts:arckit.stakeholders`, `/prompts:arckit.wardley`, `/prompts:arckit.sobc`, `/prompts:arckit.risk`
...
- `/prompts:arckit.requirements` -- Create detailed requirements from strategy
- `/prompts:arckit.roadmap` -- Expand strategic timeline into detailed roadmap
- `/prompts:arckit.diagram` -- Create architecture vision diagrams
