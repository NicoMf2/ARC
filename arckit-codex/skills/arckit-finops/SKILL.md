...
# /prompts:arckit.finops - FinOps Strategy Command
...
Use `/prompts:arckit.finops` after completing:

1. Requirements (`/prompts:arckit.requirements`) - for scale and budget constraints
2. Architecture diagrams (`/prompts:arckit.diagram`) - for resource topology
3. DevOps strategy (`/prompts:arckit.devops`) - for infrastructure patterns
...
> **Tip**: Users can customize templates with `/prompts:arckit.customize finops`
...
"⚠️ Cannot find requirements document (ARC-*-REQ-*.md). Please run `/prompts:arckit.requirements` first. FinOps strategy requires NFRs for budget and scale requirements."
...
```markdown
---
**Generated by**: ArcKit `/prompts:arckit.finops` command
**Generated on**: [DATE]
